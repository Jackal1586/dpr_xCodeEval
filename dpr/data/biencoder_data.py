...
=======
	...
	def __getitem__(self, index) -> BiEncoderSample:
		...
		r.hard_negative_passages = [create_passage(ctx) for ctx in hard_negative_ctxs]
		return r
>>>>>>> 93d3c359c81d261899b27067890e0f67fc4dbb13
...
class JsonlRetDataset(JsonQADataset):
	def __init__(
		self,
		file: str,
		selector: DictConfig = None,
	...
	def calc_total_data_len(self):
		...
		if self.total_data_size < 0:
			...
			for _file in self.data_files:
				with open(_file) as jsonl_reader:
					for l in jsonl_reader:
						cnt+=1
						try:
							json.loads(l)
						except Exception:
							continue
		self.total_data_size = cnt
		logger.info("total_data_size=%d", self.total_data_size)
		return self.total_data_size
	...
	def convert_sample_from_xcodeeval_to_fbdpr(self, xcodeeval_sample):
		return {
				...
				"positive_ctxs": [{
					"title": f"{i}",
					"text": x["source_code"],
				} for i, x in enumerate(xcodeeval_sample["positive_code"])],
				"negative_ctxs": [],
				"hard_negative_ctxs": [{
					"title": f"{i}",
					"text": x["source_code"],
				} for i, x in enumerate(xcodeeval_sample["negative_code"])]
			}
	...
	def load_data(self, start_pos: int = -1, end_pos: int = -1):
				...
				with open(_file) as jsonl_reader:
					for l in jsonl_reader:
						try:
							...
							raise
							#continue
						self.data.extend([self.convert_sample_from_xcodeeval_to_fbdpr(l)])
			logger.info("Jsonl loaded data size %d ", len(self.data))
			return
		...
		for _file in self.data_files:
			...
			with open(_file) as jsonl_reader:
				for jline in jsonl_reader:
					if start_pos <= global_sample_id < end_pos:
						self.data.append(self.convert_sample_from_xcodeeval_to_fbdpr(jline))
					try:
						l = json.loads(l)
					except Exception:
						continue
...
class JsonlRetDataset(JsonQADataset):
    ...
    def convert_sample_from_xcodeeval_to_fbdpr(self, xcodeeval_sample):
        return {
                "question": xcodeeval_sample["source_code"],
                "question_id": xcodeeval_sample["pid"],
                "answers": [],
                "positive_ctxs": [{
                    "title": f"{i}",
                    "text": x["source_code"],
    ...
    def load_data(self, start_pos: int = -1, end_pos: int = -1):
        ...
        if start_pos < 0 and end_pos < 0:
            for file in self.data_files:
                with jsonlines.open(file, mode="r") as jsonl_reader:
                    self.data.extend([self.convert_sample_from_xcodeeval_to_fbdpr(l) for l in jsonl_reader])
            return
