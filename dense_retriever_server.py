...
from dpr.utils.model_utils import get_model_obj as load_model_obj
...
# @hydra.main(config_path="conf", config_name="dense_retriever_server")
def load_dpr(cfg: DictConfig):
    ...
    model_to_load = load_model_obj(encoder)
...
def _search(question, n_docs):
    ...
    questions_tensor = retriever.generate_question_vectors([question], query_token=None)
    # get top k results
    ...
    #     print(_search(dt["text"], 5))
    # return {"retriever": retriever, "ctxs": all_passages}


@hydra.main(config_path="conf", config_name="dense_retriever_server")
def main(cfg: DictConfig):
    # with open("conf/dense_retriever_server.yaml")
    # load_dpr()
...
if __name__ == "__main__":
    main()
    # # print(dpr_retriever__)
    # with open("dpr_deps/v3_cpp_qas_test_null.json") as rp:
    #     test_data = json.load(rp)

    # ctxs_complete_list__ = {}

    # # print(json.dumps(ctxs_complete_list__[0], indent=4))
